...
if ( ! function_exists( 'be_theme_woocommerce_setup' ) ) :
...
// Add be_theme wrappers
add_action('woocommerce_before_main_content', 'be_theme_wrapper_start', 10);
add_action('woocommerce_after_main_content', 'be_theme_wrapper_end', 10);
function be_theme_wrapper_start() {
  echo '<main id="main" class="site-main" role="main">';
}
function be_theme_wrapper_end() {
  echo '</main>';
}
/*
Place a cart icon with number of items and total cost in the menu bar.
*/
function be_theme_woomenucart($menu, $args) {
	...
	ob_start();
		global $woocommerce;
		$viewing_cart = __('View your shopping cart', 'be_theme');
		$start_shopping = __('Start shopping', 'be_theme');
		$cart_url = $woocommerce->cart->get_cart_url();
		$shop_page_url = get_permalink( woocommerce_get_page_id( 'shop' ) );
		$cart_contents_count = $woocommerce->cart->cart_contents_count;
		$cart_contents = sprintf(_n('%d item', '%d items', $cart_contents_count, 'be_theme'), $cart_contents_count);
...
add_filter('wp_nav_menu_items','be_theme_woomenucart', 10, 2);
